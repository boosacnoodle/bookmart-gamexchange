    ...
    <>
      ...
      <section className="product-detail premium-product-detail">
        ...
        <div className="product-info">
          ...
          <div className="counter-actions" aria-label="Product actions">
            <a className="button button-secondary" href={`mailto:hello@bookmart.ie?subject=${encodeURIComponent(`Question about ${product.title}`)}&body=${encodeURIComponent(`Hi Bookmart,\n\nI have a question about ${product.title} (${product.sku}).\n\n`)}`}>Make enquiry</a>
            <a className="button button-secondary" href="https://www.google.com/maps/search/?api=1&query=73+Talbot+Street%2C+Dublin+1%2C+D01+C861">Collect in shop</a>
          </div>
          <p>{product.shortDescription}</p>
          <div className="trust-strip"><span>Actual-item listing</span><span>Click & collect</span><span>Irish shipping rules</span></div>
          <div className="fulfilment-panel">
            <h2>Delivery and collection</h2>
            <div>
              <strong>Click & collect</strong>
              <span>Available from 73 Talbot Street, Dublin 1.</span>
            </div>
            <div>
              <strong>Delivery</strong>
              <span>{product.category === "RARE_COLLECTIBLE" ? "Confirm postage before dispatch for rare or fragile items." : "Irish shipping available where the item is eligible."}</span>
            </div>
          </div>
          <dl className="detail-list">
            <dt>Condition</dt><dd>{conditionLabel(product.conditionGrade)}</dd>
            <dt>Category</dt><dd>{categoryLabel(product.category)}</dd>
            ...
            <dt>Reference</dt><dd>{product.isbn ?? product.ean ?? product.sku}</dd>
