...
export async function createCheckoutSession(input: CheckoutSessionInput): Promise<CheckoutSessionResult> {
  ...
  return { id: session.id, url: session.url ?? `${appUrl()}/order-confirmation?order=${encodeURIComponent(input.orderNumber)}`, paymentIntentId: typeof session.payment_intent === "string" ? session.payment_intent : null };
}
...
export async function createStripeRefund(paymentIntentId: string, amountMinor: number, reason: string, idempotencyKey: string) {
  if (stripeRuntimeStatus().testAdapter) {
