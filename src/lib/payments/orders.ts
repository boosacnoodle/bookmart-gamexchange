  ...
  for (const reservation of expired) {
    await db.$transaction([
      ...
      db.product.updateMany({
        where: { id: reservation.productId, inventoryState: "RESERVED" },
        data: { inventoryState: "PUBLISHED" },
      }),
    ]);
  }
...
function validateBasketLines(lines: BasketLineInput[]) {
  ...
  for (const line of normalized) {
    if (!line.slug || !Number.isInteger(line.quantity) || line.quantity < 1)
      throw new CheckoutError("Basket quantity is invalid.");
  }
  return normalized;
}
...
function assertAvailable(items: { line: BasketLineInput; product: Product }[]) {
  for (const { line, product } of items) {
    ...
    if (product.quantity === 1 && line.quantity > 1)
      throw new CheckoutError(`${product.title} is a one-copy item.`);
    if (line.priceMinor !== undefined && line.priceMinor !== product.priceMinor) {
      throw new CheckoutError(
        `${product.title} has changed price. Refresh your basket before checkout.`,
      );
    }
  }
}
...
export async function createCheckout(input: CheckoutInput) {
  ...
  const deliveryMethod: DeliveryMethod =
    input.deliveryMethod === "collect" ? "CLICK_AND_COLLECT" : "SHIP_IE";
  ...
  const order = await db.$transaction(
    async (tx) => {
      ...
      if (!quote.available)
        throw new CheckoutError(quote.reason ?? "Selected delivery method is not available.");
      ...
      for (const { line, product } of items) {
        ...
        if (result.count !== 1)
          throw new CheckoutError(`${product.title} was just reserved by another customer.`);
        updated.push(product);
      }

      const subtotal = items.reduce(
        (sum, item) => sum + item.product.priceMinor * item.line.quantity,
        0,
      );
      const created = await tx.order.create({
        data: {
          ...
          deliveryMethod,
          shippingAddress:
            input.deliveryMethod === "ship-ie" ? (input.address ?? {}) : Prisma.JsonNull,
      ...
      for (const product of updated) {
        await tx.inventoryReservation.create({
          data: { productId: product.id, orderId: created.id, quantity: 1, expiresAt },
        });
      }
      return created;
    },
  ...
  const session = await createCheckoutSession({
    ...
    lineItems: order.items.map((item) => ({
      ...
      unitAmountMinor: item.priceMinor,
    })),
    ...
    data: {
      ...
      reservations: {
        updateMany: {
          ...
          data: { checkoutSessionId: session.id, paymentIntentId: session.paymentIntentId },
        },
      },
    },
    include: { items: true },
...
export async function markOrderPaidByCheckoutSession(
  ...
  const paid = await db.$transaction(async (tx) => {
    ...
    await tx.inventoryReservation.updateMany({
      where: { orderId: order.id, state: "ACTIVE" },
      data: {
        state: "CONVERTED",
    ...
    for (const item of order.items) {
      await tx.product.update({
        ...
        data: { inventoryState: "SOLD", quantity: 0 },
      });
    }
    return updated;
  });
  await sendOrderEmail(
    paid.deliveryMethod === "CLICK_AND_COLLECT"
  ...
  const failed = await db.$transaction(async (tx) => {
    ...
    for (const item of order.items) {
      await tx.product.updateMany({
        where: { id: item.productId, inventoryState: "RESERVED" },
        data: { inventoryState: "PUBLISHED" },
      });
    }
    return updated;
