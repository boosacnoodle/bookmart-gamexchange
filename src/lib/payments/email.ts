...
function bodyFor(template: EmailTemplate, order: OrderWithItems) {
  const lines = [
    ...
    "",
    ...order.items.map((item) => `- ${item.title} (${item.skuSnapshot}) - ${formatMoney(item.priceMinor)}`),
    ""
  ];
  if (template === "ready-for-collection" || template === "click-and-collect-confirmation") {
    lines.push("Collection: bring your order number to the Talbot Street shop during configured opening hours.");
  }
...
export async function sendOrderEmail(template: EmailTemplate, order: OrderWithItems) {
  ...
  try {
    ...
    await db.emailEvent.create({ data: { orderId: order.id, recipient: order.customerEmail, template, subject, status: "SENT", provider: status.provider, sentAt: new Date() } });
