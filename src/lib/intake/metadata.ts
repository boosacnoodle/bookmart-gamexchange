  ...
  "9780141187761": [
    {
      ...
      description:
        "Standard catalogue metadata for a fictional rapid-listing demo. Condition must be entered by staff.",
      coverImageUrl:
  ...
  "071171200131": [
    {
      ...
      description:
        "Standard catalogue metadata only. Disc, manual and case condition must be entered by staff.",
      coverImageUrl:
  ...
  "5099746259120": [
    {
      ...
      subjects: ["Jazz", "Vinyl"],
      description:
        "Catalogue-level music metadata. Sleeve and record condition must be entered by staff.",
      coverImageUrl:
...
export class OpenLibraryProvider implements MetadataProvider {
  ...
  async lookup(parsed: ParsedBarcode, signal?: AbortSignal): Promise<MetadataCandidate[]> {
    ...
    const response = await fetch(`https://openlibrary.org/isbn/${parsed.normalized}.json`, {
      signal,
    });
    ...
    const authorNames = await Promise.all(
      authors.slice(0, 3).map(async (author) => {
        const key =
          typeof author === "object" && author && "key" in author ? String(author.key) : "";
        if (!key) return null;
        try {
    ...
    const languageKeys = Array.isArray(data.languages)
      ? data.languages
          .map((language) =>
            typeof language === "object" && language && "key" in language
              ? String(language.key).split("/").pop()
              : null,
          )
          .filter(Boolean)
      : [];
    return [
      {
        ...
        platform: null,
        region: null,
        format: Array.isArray(data.physical_format)
          ? String(data.physical_format[0] ?? "")
          : typeof data.physical_format === "string"
...
export function metadataProviders() {
  const defaultPriority =
    process.env.NODE_ENV === "production" ? "openlibrary" : "openlibrary,demo";
  const priority = (process.env.METADATA_PROVIDER_PRIORITY ?? defaultPriority)
    .split(",")
    .map((item) => item.trim());
  ...
  return priority.flatMap((name) =>
    available.filter((provider) => provider.name.toLowerCase().includes(name.toLowerCase())),
  );
}
...
export async function lookupMetadata(input: string): Promise<MetadataLookupResult> {
  ...
  if (!parsed.valid)
    return {
      ...
      candidates: [],
      ambiguous: false,
      errors: [parsed.error ?? "Invalid barcode."],
    };
  const errors: string[] = [];
  ...
  return {
    parsed,
    candidates: unique,
    ambiguous: unique.length > 1 || unique.some((candidate) => candidate.confidence < 0.85),
    errors,
  };
}
