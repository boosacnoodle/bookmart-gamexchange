...
export type DetectedItemType =
  "Book" | "Game" | "Console" | "Music" | "Film" | "Collectible" | "Other";
...
export function categoryRoute(category: ProductCategory, platform?: string | null) {
  if (category === "BOOKS") return "/library";
  if (category === "RARE_COLLECTIBLE") return "/curiosity-cabinet";
  if (category === "MUSIC_FILM" || category === "VINYL") return "/sound-vision";
  if (category === "GAMES" || category === "CONSOLES") {
    return platform ? `/arcade?platform=${encodeURIComponent(platform)}` : "/arcade";
  }
  return "/curiosity-cabinet";
}

export function publicDepartmentFor(
...
export function classifyCandidate(
  ...
  const evidence = [candidate.title, candidate.edition, ...(candidate.subjects ?? [])]
    .map(normalized)
    .join(" ");
  ...
  if (
    ...
    !/(collectible|signed|first edition|antiquarian|rare)/.test(evidence) &&
    !overrides.rare
  ) {
    category = "BOOKS";
  }
