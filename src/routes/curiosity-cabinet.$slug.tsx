...
const ROOM = "curiosity" as const;

export const Route = createFileRoute("/curiosity-cabinet/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    ...
    const title = `${item.title} — ${item.maker} | The Curiosity Cabinet, Bookmart & GameXchange`;
    const description = [
      `${item.condition} second-hand copy of ${item.title} by ${item.maker} on the ${item.shelf} shelf in The Curiosity Cabinet, Dublin.`,
    ...
    <div className="min-h-screen bg-ink">
      ...
      <main className="mx-auto max-w-[900px] px-6 pt-32 pb-24 sm:px-8">
        ...
        <p className="shop-meta mt-8 text-brass/60">
          ...
          >
            Back into The Curiosity Cabinet
          </Link>
        </p>
      </main>
    </div>
