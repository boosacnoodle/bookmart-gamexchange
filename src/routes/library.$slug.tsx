...
export const Route = createFileRoute("/library/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const title = `${item.title} — ${item.maker} | The Library, Bookmart & GameXchange`;
    const description = [
      `${item.condition} second-hand copy of ${item.title} by ${item.maker} on the ${item.shelf} shelf in The Library, Dublin.`,
