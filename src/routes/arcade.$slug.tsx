...
const ROOM = "arcade" as const;

export const Route = createFileRoute("/arcade/$slug")({
  loader: async ({ params }) => {
    const item =
  ...
  head: ({ loaderData }) => {
    ...
    const title = `${item.title} — ${item.maker} | The Arcade, Bookmart & GameXchange`;
    const description = [
      `${item.condition} second-hand copy of ${item.title} by ${item.maker} on the ${item.shelf} shelf in The Arcade, Dublin.`,
